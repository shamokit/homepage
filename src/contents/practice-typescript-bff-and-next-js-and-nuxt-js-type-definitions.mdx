...
- 2022-04-24 第四章を読み終わった。
- 2022-04-25 第五章を読み終わった。
- 2022-04-26 第六章6-2まで読み終わった。
...
let list: Array<number> = [10] //こっちの書き方は使わんかも
```
...
const x = user.enquete['test'] //存在しないプロパティ(undefined)だがAnswerに型推論されている
const y = user.enquete['exercise_habits']
```
---
```ts
type Answer = 'A' | 'B' | 'C'
...
## 第五章 TypeScriptの型システム

interfaceとtypeのちがいなど
...
これでvalueにstring以外を入れたときエラーになる。
Tの部分はTUKなどがよくつかわれる。

Genericsには以下のように初期値も設定できる。
...
Genericsにextendsシグネチャを付与すると指定可能な型を制約できる。
...
これでTにはstringもしくはnumberだけしか入れられないようになる。
...
```ts
const ComponentX = <T>(props: T) => {
  return ({
    width: props
  })
}
const componentX = ComponentX(100)
const componentY = ComponentX("100px")
```

extendsも使える。

```ts
const ComponentX = <T extends number>(props: T) => {
  return ({
    width: props
  })
}
const componentX = ComponentX(100) // ok
const componentY = ComponentX("100px") // error
```
...
const catName = pick(cat, "name") //string
const catAge = pick(cat, "age") //number
const catVaccinated = pick(cat, "vaccinated") //boolean
const catTest = pick(cat, "test") //error
```
...
type isString<T> = isType<T, string>
type isStringA = isString<"AAA"> //true
type isStringB = isString<0> //false
...
}[keyof T] //[keyof T]がないとPersonの各キーに型推論されたものが返る。
type StringKeys = Filter<Person, string> //name
type NumberKeys = Filter<Person, number> //age
type BooleanKeys = Filter<Person, boolean> //vaccinated
type StringArrayKeys = Filter<Person, string[]> //hobby
type ArrayKeys = Filter<Person, []> // never
const person:Pick<Person, StringKeys> = {
  name: "しゃもきっと",
  age: 29, // error
  vaccinated: true, // error
  hobby: ["fishing"] // error
}
```
...
```ts
const greet = (name: string, genuine: boolean) => `Hello! ${name} ${genuine ? '!' : '?'}`
const greet1 = () => `Hello!`

// 引数の一つ目の型を返す
type Return1<T> = T extends (...args: [infer U, ...any[]]) => any ? U : never
type Return1Test1 = Return1<typeof greet> //string
type Return1Test2 = Return1<typeof greet1> //unknown

// 引数の二つ目の型を返す
type Return2<T> = T extends (...args: [any, infer U, ...any[]]) => any ? U : never
type Return2Test1 = Return2<typeof greet> //boolean
type Return2Test2 = Return2<typeof greet1> //boolean

// 引数全ての型を返す
type Return3<T> = T extends (...args: infer U) => any ? U : never
type Return3Test1 = Return3<typeof greet> //[name: string, genuine: boolean]
type Return3Test2 = Return3<typeof greet1> //[]

// 返り値の型を返す
type Return4<T> = T extends (...args: any[]) => infer U ? U : never
type Return4Test1 = Return4<typeof greet> //string
type Return4Test2 = Return4<typeof greet1> //string
```
